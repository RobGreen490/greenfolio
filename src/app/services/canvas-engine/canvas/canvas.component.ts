...
export class CanvasComponent implements AfterViewInit, OnDestroy{
  ...
  // Receives the parent page's draw function so CanvasComponent can call it each animation frame.
  ...
  ngOnDestroy(): void {
    this.running = false;
    // if an animation exist, cancel it on destroy to avoid memory leak
    if(this.animationId !== undefined)
      cancelAnimationFrame(this.animationId);

    window.removeEventListener('mousemove', this.handleMouseMove);
    // console.log("Destroying old canvas..")
  }
